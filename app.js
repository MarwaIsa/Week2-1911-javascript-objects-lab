...
*/
// Add a new property called "difficulty"
game.difficulty = ["Med"]; 
console.log(game);
...
game.party.splice(1,0,pokemon[4],pokemon[7],)
console.log(game.party)
...
game.party.splice(0,2,pokemon[8],pokemon[10],pokemon[11])
console.log(game.party)
...
game.party.forEach(PokémonName => {
  console.log(PokémonName.name)
...
console.log(game.party)
  ...
  /*
Exercise 11
1. Copy the `catchPokemon` method that you just wrote above, and paste it below. Modify it so that it also decreases the number of
...
For example, if five gym objects have a value of `true` on their `completed` property and three gym objects have a value of `false` on
 their `completed` property, the logged value would be: `{ completed: 5, incomplete: 3 }`.
...
game.gymStatus = function gymStatus() {
  game.gyms.forEach(eachGyms => {
    if (eachGyms.completed === true) {
      gymTally.completed +=1
    }
    if (eachGyms.completed === false) {
...
game.partyCount = function partyCount() {
  game.party.forEach(CountPokemon => {
    counter+=1 
  });
  return counter
}
